...
declare function local:create-collection($collection-name, $parent-collection-key) {
    ...
    return 
        ...
        then
            ...
            return httpclient:post(xs:anyURI($base-uri || "/collections" || $api-key-parameter || "&amp;format=atom"), $serialized-content, true(), ())/httpclient:body/*
        else ()
};
...
let $collection-name := "test claudius 21"
...
return ( 
    ...
    ,
    httpclient:post(xs:anyURI($base-uri || "/collections" || $api-key-parameter || "&amp;format=atom"), $serialized-content, true(), $request-headers)
    ,
    httpclient:post(xs:anyURI($base-uri || "/collections" || $api-key-parameter || "&amp;format=atom"), $serialized-content, true(), $request-headers)
)
...
(:return local:create-collection("test claudius 108", ()) :)