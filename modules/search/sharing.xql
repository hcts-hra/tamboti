            ...
            <aaData>
                {
                    ...
                    let $who :=
                        if ($target = 'USER')
                        then security:get-human-name-for-user($username)
                        else $ace/@who
                    order by $username
                    
                    return
                        element json:value {
                            if(xs:integer($acl/@entries) eq 1)
                            then attribute json:array {true()}
                            else ()
                            ,
                            <json:value>{text{$ace/@target}}</json:value>,
                            <json:value>{text{$who}}</json:value>,
                            <json:value>{$username}</json:value>,
...
declare function local:get-folder-files ($upload-folder as xs:string ) {
    ...
    return
        <aaData json:array="true">
            {file:directory-list($upload-folder,'*.xml')}
        </aaData>    
};
...
declare function local:list-collection-contents($collection as xs:string, $user as xs:string) {
    let $subcollections := 
        ...
        where security:user-has-access(security:get-user-credential-from-session()[1], concat($collection, "/", $child), "r..")
        
        return concat("/", $child)
    let $resources :=
        for $r in xmldb:get-child-resources($collection)
        where security:user-has-access(security:get-user-credential-from-session()[1], concat($collection, "/", $r), "r..")
            ...
            <items>
            {
                ...
                let $path := 
                    if ($isCollection) then
                        ...
                        concat($collection, "/", $resource)
                where security:user-has-access(security:get-user-credential-from-session()[1], $path, "r..")
                order by $resource ascending
                
                return
                    let $permissions := 
                        if ($isCollection)
                        then security:get-resource-permissions($path)
                        else security:get-resource-permissions(concat($collection, "/", $resource))
                    let $owner := 
                        if ($isCollection)
                        then security:get-owner($path)
                        else security:get-owner(concat($collection, "/", $resource))
                    let $group :=
                        if ($isCollection)
                        then security:get-group($path)
                        else security:get-group(concat($collection, "/", $resource))
                    let $lastMod := 
                        let $date :=
                            if ($isCollection)
                            then xmldb:created($path)
                            else xmldb:created($collection, $resource)
                        return
            ...
            (
                ...
                return
                    <json:value json:array="true">     
                        ...
                        <lastmodified>
                            {
                                if (xs:date($modified) = current-date())
                                then format-dateTime($modified, "Today [H00]:[m00]:[s00]")
                                else format-dateTime($modified, "[M00]/[D00]/[Y0000] [H00]:[m00]:[s00]")
                            }
                        </lastmodified>
                    </json:value>
            )
...
<json:value>
    {
        ...
        return
            if ($collection != "")
            then local:get-sharing($collection)
            else
                if (request:get-parameter("file",()))
                then local:get-attached-files(request:get-parameter("file",()))
                else
                    if (request:get-parameter("upload-folder",()))
                    then local:resources(request:get-parameter("upload-folder",()),security:get-user-credential-from-session()[1])
                    else local:empty()
    }
</json:value>
