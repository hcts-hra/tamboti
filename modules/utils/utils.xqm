    ...
    if (xmldb:collection-available($collection-uri || "/VRA_images")) then
        ...
        try {
            xmldb:create-collection($collection-uri, "/VRA_images"),
	            (
	                sm:chown(xs:anyURI($collection-uri || "/VRA_images"), sm:get-permissions(xs:anyURI($collection-uri))/*/@owner/data(.)),
    ...
    let $result :=
        let $resource-name := 
            switch ($resource-format)
                (: VRA  :)
                case "VRA"
                return util:document-name(collection($source-collection)//vra:work[@id = $resource-id][1])
                (: TEI :)
(:                case "TEI" return:)
(:                    util:document-name(collection($source-collection)//vra:work[@id = $resource-id][1]):)
                (: default: MODS:)
                default return util:document-name(collection($source-collection)//mods:mods[@ID = $resource-id][1])
        return
            try {
                (: move resource       :)
                (: if VRA we have to move image recods and binaries as well :)
                if ($resource-format = "VRA")
                then 
                    try {
...
(:                                            util:log("INFO", "testing availability:"  || $vra-images-source-collection || "/" || $binary-name),:)
                                    if (util:binary-doc-available($vra-images-source-collection || "/" || $binary-name))
                                    then
                                        (
