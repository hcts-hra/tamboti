...
declare function nameutil:format-name($name as element()) as xs:string* {
    ...
    let $sortFirst :=
    	...
    	if ($namePart-elements[not(@lang = $nameutil:eastern-languages)]/text())
    	then
    		...
    		then $namePart-elements[not(@lang = $nameutil:eastern-languages)][@type = 'family'][1]/text()
    		else $namePart-elements[not(@lang = $nameutil:eastern-languages)][1]/text()
    	else
	...
	let $sortLast :=
	    	...
	    	(: Insert commas before Western given names. :)
	    		concat(', ', $namePart-elements[not(@lang = $nameutil:eastern-languages)][@type eq 'given'][1]/text())
	    	else
		    	if ($namePart-elements[@transliteration]/text())
		    	then
